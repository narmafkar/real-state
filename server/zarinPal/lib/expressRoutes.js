...
module.exports = {
  appRun: function (port) {
    app.listen(port, () =>
      console.log(
        "\
Zarinpal NodeJS payment app is running on port: " +
          port +
...
You also have the option to change the port in appConfig.js file.\
  ...
  userGUI: function () {
    ...
    app.post("/x/handle/accepted", function (req, res) {
      mySqlLib.addTransaction(
        "Pending",
        req.body.acfirstname,
        req.body.aclastname,
        req.body.acemail,
        req.body.acphone,
        req.body.acamount,
        req.body.acdescription,
        // req.body.acclientId,
        function (getpid) {
          zarinpalLib.request(
            req.body.acamount,
  ...
  adminPanel: function () {
      ...
      if (req.session.adminValidate) {
        mySqlLib.getTransactions(function (data) {
          res.render("admin/transactions", {
            zpTitle: appConfig.appTitle,
            adminPanelPath: appConfig.adminPanelPath,
    ...
    app.post("/" + appConfig.adminPanelPath + "/login", function (req, res) {
      if (
        appConfig.adminUsername === req.body.zpAdminUsername &&
        appConfig.adminPassword === req.body.zpAdminPassword
      ) {
